...
> **Date:** 2026-02-17
> **Version:** Covers Phases 0–8C (model proxy, CONNECT proxy, session auth, agent UI proxy, provisioning scripts).
...
### 6D. CONNECT Proxy (Phase 8B)

| Threat | Description | Controls in Place | Residual Risk |
|--------|------------|-------------------|---------------|
| **S** Spoofing | Agent spoofs its identity to bypass per-crab rules | CONNECT handler resolves `crabId` from bearer token before rule lookup | Token theft enables rule bypass for that crab |
| **T** Tampering | Admin manipulates ConnectRules to deny legitimate traffic | Admin-only route (requireAdmin); change logged to tides (gap — admin audit incomplete) | Malicious insider can silently block an agent |
| **I** Info disclosure | HTTPS tunnel content visible to Shell | HTTPS content is end-to-end encrypted; Shell sees host:port only | N/A — by design |
| **D** DoS | Flood of CONNECT requests exhausts file descriptors | No CONNECT-specific rate limit (only `/v1/execute` is rate-limited) | Gap: G11 — add rate limiting to CONNECT handler |
| **E** Elevation | Bypass domain rules via DNS rebinding | Rules check resolved IP, not just hostname; same TOCTOU window as SSRF guard | DNS rebinding window remains; see G6 |
| **E** Elevation | Tunnel to internal sand_bed services | SSRF check applied to CONNECT destinations; loopback and RFC-1918 blocked | Agents could potentially CONNECT to sibling containers by name if rules allow |

### 6E. Session Auth + Agent UI Proxy (Phase 8C)

| Threat | Description | Controls in Place | Residual Risk |
|--------|------------|-------------------|---------------|
| **S** Spoofing | Forge session cookie without knowing `ADMIN_API_KEY` | HMAC-SHA256 signature over payload; timing-safe comparison | N/A if key is strong (64-char hex) |
| **S** Spoofing | Replay expired cookie | TTL embedded in payload (`iat`); `verifySession()` checks expiry | N/A |
| **T** Tampering | Modify cookie payload (e.g. extend TTL) | HMAC covers the payload; any modification invalidates signature | N/A |
| **I** Info disclosure | Cookie theft via XSS | Cookie is `HttpOnly` — not accessible via JavaScript | N/A |
| **I** Info disclosure | Cookie theft via network sniffing | Cookie is `Secure` in production (requires TLS reverse proxy) | Plain HTTP in dev — acceptable tradeoff |
| **I** Info disclosure | Agent UI content exposed without auth | `requireSession` preHandler on all `/agents/:name/*` routes and WS upgrade handler | If WebSocket upgrade handler skips check — code review critical |
| **D** DoS | Flood of login attempts with wrong key | No rate limiting on `POST /v1/auth/login` | Gap: G12 — add rate limiting to login route |
| **E** Elevation | Access another agent's UI via path traversal | Agent name validated against crab record in DB; 404 if not found | N/A — DB lookup gates each request |

### 6F. Provisioning Scripts (Phase 8C)

| Threat | Description | Controls in Place | Residual Risk |
|--------|------------|-------------------|---------------|
| **S** Spoofing | `clawbot-add.sh` registers a crab with a predictable name | Token is random 64-char hex generated by HermitClaw API; name is descriptive only | N/A |
| **T** Tampering | Shell injection via `<name>` parameter in scripts | **KNOWN GAP** — name is not validated to `[a-z0-9-]` before use in shell commands | P2 item in security backlog; mitigated by fact that scripts are admin-only |
| **I** Info disclosure | Token written to disk in plaintext | File is `chmod 600`; gitignored; only owner-readable | Host compromise exposes all tokens |
| **D** DoS | `clawbots-sync.sh` destroys volumes of running agents | Requires explicit `--destroy` flag | Human error possible |
| **E** Elevation | Token file readable by other host users | `chmod 600` + single-user home server assumption | Multi-user host would need stronger isolation |
...
| G11 | **MEDIUM** | No rate limiting on HTTP CONNECT handler — a compromised agent could open unlimited tunnels exhausting file descriptors | Apply `@fastify/rate-limit` pattern to CONNECT handler (same as `/v1/execute`) |
| G12 | **LOW** | No rate limiting on `POST /v1/auth/login` — allows password-spraying against the admin key | Apply rate limit (e.g. 10 req/min per IP) to login route |
...
*Threat model maintained alongside codebase. Re-review on every new phase (next: 8D inbound routing) as attack surface changes.*
